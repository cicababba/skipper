...
You are working inside a **Skipper**: a personal project + knowledge workspace managed by [Skipper](https://github.com/cicababba/skipper). Think of it as a folder-based OS where notes, captured engineering knowledge, and real code projects live side by side in one integrated environment.

## Finding the Skipper root

You may be launched from the Skipper root itself, or from deep inside a subdirectory (e.g. `Projects/my-api/`). **Do not assume your current working directory is the Skipper root.**

To find the Skipper root: starting from your current working directory, walk **up** the directory tree until you find a directory that contains **all of**: `CLAUDE.md` (this file), `.skipper/`, `Daily/`, and `Skills/`. That directory is `<skipper_root>`.

**Every path mentioned in this document and in the session skills** — `Daily/`, `Skills/`, `Projects/`, `.skipper/`, etc. — resolves against `<skipper_root>`, never against your current working directory. When you read, write, or list any of these, use the absolute path `<skipper_root>/<path>`.

## Prime directive

- `.skipper/` belongs to **Skipper**. Everything else belongs to the **user**.
- Never touch `.skipper/` — it holds internal state (knowledge atoms, vector index, settings).

## Directory map

| Path                     | Owner              | Purpose                                                                 |
|--------------------------|--------------------|-------------------------------------------------------------------------|
| `.skipper/`             | Skipper           | Knowledge atoms, vector index, settings. Never modify.                  |
...
| `.trash/`                | Skipper           | Soft-deleted files. Treat as recoverable; don't auto-empty.             |
| `CLAUDE.md` (this file)  | Skipper           | Workspace orientation. You are reading it.                              |
...
skipper knowledge list            # List atoms in the pending queue
skipper knowledge review          # Interactively triage pending atoms
skipper projects register         # Install the post-commit hook in a repo
...
The skill drafts the atom, **shows it to the user**, waits for confirm/edit, then writes it to the pending review queue. The user accepts it later from Skipper → Knowledge.
...
**Before responding to the user's first message**, you must check for an orphaned open session. This applies whether you were launched from the Skipper root or from inside a project subdirectory — always resolve paths against `<skipper_root>` as described above.

1. List files in `<skipper_root>/Daily/` matching `*.md` and find the most recent by filename (they are timestamped `YYYY-MM-DD_HH-mm-ss.md`).
...
- Never delete `.skipper/raw/` — captured knowledge atoms, not regenerable.
- User notes are *not* regenerable. When in doubt about ownership, ask.
- Prefer moving files to `<skipper_root>/.trash/` over hard-deleting.

---

*Skipper — by [Mike Gazzaruso / NextEpochs](https://github.com/mikegazzaruso). Learn more: https://github.com/cicababba/skipper*
