...
- **Project**: auto-detect by walking up `cwd` to find `.skipper/`, then checking if cwd sits under `<skipper_root>/Projects/<name>/`. If yes, that's the default project. Otherwise, ask the user once: *"Su quale progetto lo attribuiamo?"*
...
1. `command -v skipper` → use `skipper`
2. macOS: `/Applications/Skipper.app/Contents/Resources/web/apps/web/skipper.bundle.cjs` → use `node "<path>"`
   Windows: `%LOCALAPPDATA%\Programs\Skipper\resources\web\apps\web\skipper.bundle.cjs` → use `node "<path>"`
3. `<skipper_root>/Projects/skipper/packages/cli/src/index.ts` → use `npx tsx "<path>"`
4. None → reply *"Skipper CLI not found."* and stop.
...
> Salvato nella coda di review. Apri Skipper → Knowledge per accettarlo.
> Saved to the review queue. Open Skipper → Knowledge to accept it.
