...
- **CLI**: `commander`
- **LLM completions providers** (`packages/core/src/llm/`): `claude-cli` (default — spawns the user's `claude` CLI) and `openai`. Structured/repair rounds and the plan critic are **runtime-first** (`structuredCall` in `packages/core/src/runtime/structured.ts`): a role that has a runtime runs them on its own CLI with no tools, so a non-Claude default agent never needs the `claude` binary. This completions backend is the fallback for the no-runtime path (openai) — it must only ever see Claude model aliases (or the OpenAI model).
