// Bundle the inbox poller (src/inbox-poller.ts + @nestbrain/core) into a
...
await build({
  entryPoints: ["src/inbox-poller.ts"],
  bundle: true,
  platform: "node",
  format: "cjs",
  target: "node20",
  outfile: "dist/inbox-poller.cjs",
  external: ["electron"],
  logLevel: "warning",
});
console.log("[inbox] bundled dist/inbox-poller.cjs");
