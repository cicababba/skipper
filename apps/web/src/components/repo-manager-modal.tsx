...
import { parseRepoPath } from "@skipper/shared";
...
export function RepoManagerModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  // Provider-neutral entry point (#15): follow a repo by typing owner/name, with
  // no account, discovery call or local clone involved.
  const add = () => {
    const repo = parseRepoPath(addValue);
    if (!repo) {
      setAddError(t.inbox.repos.addInvalid);
      return;
    }
    setAddError(null);
    void run(`${repo.owner}/${repo.name}`, async () => {
      const res = await window.skipper!.orchestrator.setRepoFollowed(repo.owner, repo.name, true);
              ...
              <section className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder={t.inbox.repos.addPlaceholder}
                    disabled={busyKey !== null}
                    className="flex-1 min-w-0 rounded-md border border-border bg-card-hover px-2 py-1.5 text-sm font-mono disabled:opacity-50"
                  />
                  <button
                    onClick={add}
                    disabled={busyKey !== null || addValue.trim() === ""}
                    className="flex items-center gap-1 text-[11px] font-medium px-2.5 py-1.5 rounded-md border border-accent/30 bg-accent/10 text-accent hover:bg-accent/20 transition-colors disabled:opacity-50 whitespace-nowrap"
                  >
                    <Plus size={11} />
                    {t.inbox.repos.addAction}
                  </button>
                </div>
