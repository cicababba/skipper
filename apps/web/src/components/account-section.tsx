...
// Settings → Account section. Google sign-in identifies the supporter
// license for app updates.
  ...
  return (
    <section className="mb-10">
      <h2 className="text-sm font-medium text-muted/70 uppercase tracking-wider mb-4 flex items-center gap-2">
        <UserRound size={13} className="text-muted/60" />
      ...
      <div className="p-5 rounded-xl bg-card border border-border space-y-5">
        {auth.status === "unconfigured" && (
          <div className="flex items-start gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium mb-1">{t.settings.account.freeTitle}</p>
              <p className="text-[11px] text-muted/60 leading-relaxed">
                {t.settings.account.freeDesc}
              </p>
            </div>
            <button
              disabled
        ...
        {auth.status === "signed-out" && (
          <div className="flex items-start gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium mb-1">{t.settings.account.signedOutTitle}</p>
              <p className="text-[11px] text-muted/60 leading-relaxed">
                {t.settings.account.signedOutDesc}
              </p>
            </div>
            <button
              onClick={signIn}
        ...
        {auth.status === "signing-in" && (
          <div className="flex items-center gap-3 text-sm text-muted">
            ...
            <span>{t.settings.account.waitingBrowser}</span>
            <button
        ...
        {auth.status === "error" && (
          <div className="text-sm text-red-400 flex items-center gap-3">
            <span>{t.settings.account.signInFailed(auth.error)}</span>
            <button
              onClick={signIn}
