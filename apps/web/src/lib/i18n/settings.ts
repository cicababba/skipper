...
const en = {
  ...
  orchestration: {
    ...
    runtimes: "Runtimes",
    runtimesDesc:
      "Which agent CLI each role runs on. Codex and Copilot run on their own configured models — the model rows above apply to the Claude runtime. Switching the coder's runtime mid-issue restarts the run with a recap of the work already done instead of resuming.",
...
const it: typeof en = {
  ...
  orchestration: {
    ...
    runtimes: "Runtime",
    runtimesDesc:
      "Quale CLI agente usa ogni ruolo. Codex e Copilot usano il modello configurato al loro interno — le righe dei modelli qui sopra valgono per il runtime Claude. Cambiare il runtime del coder a metà issue fa ripartire il run con un riepilogo del lavoro già fatto invece di riprenderlo.",
...
const fr: typeof en = {
  ...
  orchestration: {
    ...
    runtimes: "Runtimes",
    runtimesDesc:
      "Quel CLI agent chaque rôle utilise. Codex et Copilot tournent sur leur propre modèle configuré — les lignes de modèles ci-dessus concernent le runtime Claude. Changer le runtime du coder en cours d'issue relance le run avec un récapitulatif du travail déjà fait au lieu de le reprendre.",
...
const es: typeof en = {
  ...
  orchestration: {
    ...
    runtimes: "Runtimes",
    runtimesDesc:
      "Qué CLI de agente usa cada rol. Codex y Copilot usan su propio modelo configurado — las filas de modelos de arriba aplican al runtime de Claude. Cambiar el runtime del coder a mitad de la issue reinicia la ejecución con un resumen del trabajo ya hecho en vez de reanudarla.",
